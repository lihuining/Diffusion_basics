...
class StableDiffusionXLAdapterControlnetPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        height = height or self.default_sample_size * self.vae_scale_factor # 1024
        width = width or self.default_sample_size * self.vae_scale_factor # 1024

        height_sd1_5 = height_sd1_5 or self.default_sample_size_sd1_5 * self.vae_scale_factor_sd1_5 # 512
        width_sd1_5 = width_sd1_5 or self.default_sample_size_sd1_5 * self.vae_scale_factor_sd1_5 # 512

        original_size = original_size or (height, width) # (1024, 1024)
        target_size = target_size or (height, width) # (1024, 1024)
        ...
        ) # ,50,adapter_guidance_start,device,None
        ...
        if denoising_end is not None and type(denoising_end) == float and denoising_end > 0 and denoising_end < 1: # False
        ...
        latents_sd1_5_prior = latents_sd1_5.clone() # torch.Size([1, 4, 64, 64])
        ### stage1 得到 sd1.5和SDXL的latent initial,sd1.5经过controlnet然后vae decode，通过SDXL的vae encoder计算得到
        # num_inference_steps_sd1_5 = 0
        with self.progress_bar(total=num_inference_steps_sd1_5) as progress_bar: ### 50 ?
        ...
        latents = self.prepare_xl_latents_from_sd_1_5(latents_sd1_5_prior, latent_timestep, batch_size,
                        num_images_per_prompt, height, width, prompt_embeds.dtype, device, generator=generator, add_noise=add_noise) # torch.Size([1, 4, 128, 128])
        latents_sd1_5 = self.sd1_5_add_noise(latents_sd1_5_prior, latent_timestep, generator, device, prompt_embeds.dtype) # torch.Size([1, 4, 64, 64])
        ...
        for i in range(len(timesteps)):
            ...
            controlnet_keep.append(keeps[0] if isinstance(controlnet, ControlNetModel) else keeps)
        ### stage2 sd1.5,x-adapter,sdxl
        with self.progress_bar(total=num_inference_steps) as progress_bar: # 40
            for i, t in enumerate(timesteps):
                ...
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    ...
                    guess_mode=guess_mode,
                    return_dict=False,
                ) # down_block_res_samples:list,len is 12,mid_block_res_sample:list,len is 2
                ...
                ) # unet_output:UNet2DConditionOutput
                noise_pred = unet_output.sample # torch.Size([2, 4, 64, 64])
                hidden_states = unet_output.hidden_states # list,length is 3
                ...
                for xx in range(len(up_block_additional_residual)):
                    up_block_additional_residual[xx] = up_block_additional_residual[xx] * adapter_condition_scale # self.unet当中使用，添加到up module
                ...
                # compute the previous noisy sample x_t -> x_t-1
                latents_sd1_5 = self.scheduler_sd1_5.step(noise_pred, t_sd1_5, latents_sd1_5, **extra_step_kwargs, return_dict=False)[0] ### ? 后来没用到？
                ...
                if True:
                    ...
                    noise_pred = self.unet(
                            ...
                            cross_attention_kwargs=cross_attention_kwargs,
                            added_cond_kwargs=added_cond_kwargs,
                            up_block_additional_residual=up_block_additional_residual, # 添加adapter的作用
                            down_bridge_residuals=down_bridge_residuals,
                            return_dict=False,
                            fusion_guidance_scale=fusion_guidance_scale,
                            fusion_type='ADD',
    ...
    def get_timesteps(self, num_inference_steps, strength, device, denoising_start=None): # 50,adapter_guidance_start,device,None
